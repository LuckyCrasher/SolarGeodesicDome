...
def generate_vertices_and_faces_half(radius, center):
    r = (1.0 + np.sqrt(5.0)) / 2.0
    vertices = np.array([
        [-1.0, r, 0.0],     # 0
        [1.0, r, 0.0],      # 2
...
class BetterGeodesicDome:
    ...
    def __init__(self, subdivisions=0, radius=1, center=(0, 0, 0)):
        ...
        self.vertices, self.faces = generate_vertices_and_faces_half(radius, center)
        self.subdivide(divisions=subdivisions)

    def subdivide(self, divisions=0):
        for _ in range(divisions):
            ...
            for triangle in self.faces:
                ...
                for i, j in zip([0,1,2], [1,2,0]):
                    ...
                    mid_vertex_ab = compute_middle_vertex(vertex_a, vertex_b)
